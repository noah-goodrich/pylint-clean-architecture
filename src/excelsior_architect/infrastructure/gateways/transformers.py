...
class AddReturnTypeTransformer(cst.CSTTransformer):
    ...
    def leave_FunctionDef(self, original_node: cst.FunctionDef, updated_node: cst.FunctionDef) -> cst.FunctionDef:
        if original_node.name.value == self.function_name and not original_node.returns:
...
class TypeIntegrityTransformer(cst.CSTTransformer):
    ...
    def visit_Annotation(self, node: cst.Annotation) -> None:
        ...
        if isinstance(node.annotation, cst.Name) and node.annotation.value in self.typing_aliases:
            self.used_types.add(node.annotation.value)
        # Check Subscript (List[str])
        if isinstance(node.annotation, cst.Subscript) and isinstance(node.annotation.value, cst.Name):
