...
from excelsior_architect.domain.protocols import AstroidProtocol, StubAuthorityProtocol
...
class AstroidGateway(AstroidProtocol):
    ...
    def parse_file(self, file_path: str) -> astroid.nodes.Module | None:
    ...
    def get_node_return_type_qname(self, node: astroid.nodes.NodeNG) -> str | None:
    ...
    def _resolve_safe_origin(self, node: astroid.nodes.NodeNG) -> str | None:
        """Trace node to its origin and check if it comes from the standard library via Typeshed."""
        return self._trace_safety(node, set())

    def _trace_safety(self, node: astroid.nodes.NodeNG, visited: set[int]) -> str | None:
    ...
    def _check_iterator_safety(self, iter_node: astroid.nodes.NodeNG, visited: set[int]) -> str | None:
    ...
    def _resolve_typing_cast(self, node: astroid.nodes.Call) -> str | None:
        """Extract type from typing.cast(Type, expr)."""
        if len(node.args) >= 1:
            return self._resolve_annotation(node.args[0])
        return None

    def _discovery_fallback(self, node: astroid.nodes.NodeNG) -> str | None:
    ...
    def _discover_from_call(self, node: astroid.nodes.Call) -> str | None:
    ...
    def _discover_from_name(self, node: astroid.nodes.Name | astroid.nodes.AssignName) -> str | None:
        """Discovery logic specifically for Name/AssignName nodes."""
        if isinstance(node, astroid.nodes.AssignName):
            return None  # AssignName doesn't have lookup
        for def_node in node.lookup(node.name)[1]:
            ...
            if parent and isinstance(parent, astroid.nodes.AnnAssign):
                annotation = getattr(parent, "annotation", None)
                if annotation:
                    return self._resolve_annotation(annotation)

            # Direct annotation (fallback)
            annotation = getattr(def_node, "annotation", None)
            if annotation:
                return self._resolve_annotation(annotation)

            # Argument annotation
            parent = getattr(def_node, "parent", None)
            if parent and isinstance(parent, astroid.nodes.Arguments):
                res = self._resolve_arg_annotation(def_node, parent)
                if res:
                    return res
        return None

    def _resolve_arg_annotation(self, def_node: astroid.nodes.NodeNG, args: astroid.nodes.Arguments) -> str | None:
        ...
        try:
            ...
            if isinstance(def_node, astroid.nodes.AssignName):
    ...
    def _discover_from_attribute(self, node: astroid.nodes.Attribute) -> str | None:
        ...
        try:
            ...
            if module_name:
                res = self._try_module_lookup_and_stub(
                    module_name, class_name, class_qname, attr_name, context
                )
                if res is not None:
                    return res
            return None
    ...
    def _try_local_root_lookup(
        self,
        root: astroid.nodes.Module,
        clean_name: str,
        attr_name: str,
    ) -> str | None:
        """Resolve attribute from class found via root.lookup(clean_name)."""
        if not hasattr(root, "lookup"):
            return None
        lookup_res = root.lookup(clean_name)
        if not (lookup_res[1] and isinstance(lookup_res[1][0], astroid.nodes.ClassDef)):
            return None
        return self._resolve_attribute_in_node(lookup_res[1][0], attr_name)

    def _try_module_lookup_and_stub(
        self,
        module_name: str,
        class_name: str,
        class_qname: str,
        ...
        """Resolve via astroid module lookup, then typeshed if stdlib, then StubAuthority."""
        ...
        if lookup_res[1] and isinstance(lookup_res[1][0], astroid.nodes.ClassDef):
            class_def = lookup_res[1][0]
            res = self._resolve_attribute_in_node(class_def, attr_name)
            if res:
                return res
    ...
    def _get_stub_authority(self) -> StubAuthorityProtocol | None:
    ...
    def _project_root_from_node(self, node: astroid.nodes.NodeNG) -> str | None:
        """Infer project root from node's file (dir containing pyproject.toml or stubs/)."""
        try:
            root = node.root()
            p = getattr(root, "file", None)
            if not p:
                return None
            from pathlib import Path
            d = Path(p).resolve().parent
            for _ in range(10):
                if (d / "pyproject.toml").exists() or (d / "stubs").is_dir():
                    return str(d)
                parent = d.parent
                if parent == d:
                    break
    ...
    def _get_stub_attribute_type(
        ...
        if not stub:
            return None
        proot = self._project_root_from_node(context)
        return stub.get_attribute_type(module_name or "", class_name, attr_name, proot)
    ...
    def _resolve_method_in_node(self, class_node: astroid.nodes.ClassDef, method_name: str) -> str | None:
    ...
    def _resolve_annotation(self, anno: astroid.nodes.NodeNG) -> str | None:
    ...
    def _resolve_subscript_annotation(self, anno: astroid.nodes.Subscript) -> str | None:
    ...
    def _map_typing_to_builtin(self, qname: str, slice_node: astroid.nodes.NodeNG) -> str | None:
    ...
    def _resolve_simple_annotation(self, anno: astroid.nodes.NodeNG) -> str | None:
        """Resolve terminal annotation nodes (Name, Const, inferable)."""
        if isinstance(anno, astroid.nodes.Name):
            return self._resolve_name_annotation(anno)
        if isinstance(anno, astroid.nodes.Const) and isinstance(anno.value, str):
            return self._resolve_const_str_annotation(anno)
        return self._resolve_annotation_by_inference(anno)

    def _resolve_name_annotation(self, anno: astroid.nodes.Name) -> str | None:
        """Resolve a Name annotation (Self, inferred, or bare name)."""
        if anno.name == "Self":
    ...
    def _resolve_const_str_annotation(self, anno: astroid.nodes.Const) -> str | None:
        """Resolve a Const(str) annotation (Self, scope lookup, or literal)."""
        if anno.value == "Self":
            return self._resolve_self_type(anno)
        val = anno.value
    ...
    def _resolve_annotation_by_inference(self, anno: astroid.nodes.NodeNG) -> str | None:
        """Resolve annotation via infer(); return None if Uninferable or error."""
        try:
            for inf in anno.infer():
                if inf is not astroid.Uninferable:
    ...
    def _resolve_self_type(self, node: astroid.nodes.NodeNG) -> str | None:
        """Resolve 'Self' to the containing class qname."""
        scope: astroid.nodes.NodeNG | None = node.scope()
        while scope and not isinstance(scope, astroid.nodes.ClassDef):
            parent = getattr(scope, "parent", None)
            scope = parent.scope() if parent else None
    ...
    def _resolve_nested_annotation(self, slice_node: astroid.nodes.NodeNG) -> str | None:
    ...
    def _resolve_bool_op(self, expr: astroid.nodes.BoolOp, visited: set[int]) -> str | None:
    ...
    def _resolve_bin_op(self, expr: astroid.nodes.BinOp, visited: set[int]) -> str | None:
    ...
    def get_call_name(self, node: astroid.nodes.Call) -> str | None:
    ...
    def is_protocol(self, node: astroid.nodes.NodeNG) -> bool:
        ...
        qname: str | None = self.get_return_type_qname_from_expr(node)
    ...
    def is_protocol_call(self, node: astroid.nodes.Call) -> bool:
        ...
        if self._is_protocol_call_via_infer(node):
            return True
        return bool(self._is_protocol_call_via_receiver(node))
    ...
    def _is_protocol_call_via_receiver(self, node: astroid.nodes.Call) -> bool:
        ...
        if isinstance(node.func, astroid.nodes.Attribute):
            if isinstance(node.func.expr, astroid.nodes.Call) and self.is_protocol_call(node.func.expr):
                return True
    ...
    def _find_class_node(self, qname: str, context: astroid.nodes.NodeNG) -> astroid.nodes.ClassDef | None:
    ...
    def is_fluent_call(self, node: astroid.nodes.Call) -> bool:
        ...
        if isinstance(node.func.expr, astroid.nodes.Call) and self.is_fluent_call(node.func.expr):
            return True

        # 2. Direct match check
        try:
            receiver_qname: str | None = self.get_return_type_qname_from_expr(
                node.func.expr)
            return_qname: str | None = self.get_node_return_type_qname(node)
    ...
    def is_trusted_authority_call(self, node: astroid.nodes.Call) -> bool:
        ...
        return bool(receiver_qname and self._trusted_authority_via_receiver(node, receiver_qname))
    ...
    def is_primitive(self, qname: str) -> bool:
        ...
        if qname.startswith("builtins.") or base_name in (
            ...
            return True
        return bool(qname.startswith(("typing.", "collections.abc.")))
