...
class ImportLinterAdapter(LinterAdapterProtocol):
    ...
    def _parse_output(self, output: str) -> list[LinterResult]:
        ...
        if not results and "Broken contract" in text:
            state: dict[str, object] = {
                "current_contract": "", "results": results}
    ...
    def _handle_broken_contract_line(self, line: str, state: dict[str, object]) -> None:
        state["current_contract"] = line.strip()

    def _handle_import_violation_line(self, line: str, state: dict[str, object]) -> None:
        from typing import cast
        cast(list[LinterResult], state["results"]).append(
            LinterResult(
                "IL001",
