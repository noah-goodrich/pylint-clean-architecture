...
# Ruff config: flexible nested dict structure for configuration
RuffConfig = dict[str, object]


class RuffAdapter(LinterAdapterProtocol):
            ...
            if isinstance(value, dict) and key in merged:
                ...
                if isinstance(merged_value, dict):
                    ...
                    if key == "lint":
                        ...
                        merged_lint = cast(dict[str, object], merged_value)
                        for lint_key, lint_value in value.items():
                            if lint_key in ["select", "ignore"]:
                                # Project wins completely for select/ignore
                                merged_lint[lint_key] = lint_value
                            elif isinstance(lint_value, dict):
                                if lint_key not in merged_lint:
                                    merged_lint[lint_key] = {}
                                existing_lint_value = merged_lint[lint_key]
                                if isinstance(existing_lint_value, dict):
                                    existing_lint_value.update(lint_value)
                            else:
                                merged_lint[lint_key] = lint_value
                    else:
                        merged_value.update(value)
    ...
    def apply_fixes(
        ...
        try:
            ...
            cmd = ["ruff", "check", target_path, "--fix", "--unsafe-fixes"]
            if select_only:
                cmd.extend(["--select", ",".join(select_only)])
            else:
                ...
                lint_config = merged_config.get("lint")
