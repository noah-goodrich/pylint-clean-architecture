...
from excelsior_architect.domain.protocols import ScaffolderProtocol
from excelsior_architect.infrastructure.adapters.ruff_adapter import RuffConfig
from excelsior_architect.interface.telemetry import TelemetryPort


class Scaffolder(ScaffolderProtocol):
    ...
    def init_project(self, template: str | None = None, check_layers: bool = False) -> None:
    ...
    def _perform_tool_audit(self, template: str | None) -> None:
...
enable = ["clean-arch-classes", "clean-arch-imports", "clean-arch-layers"] # and other specific checks
    ...
    def _configure_ruff_wizard(self) -> None:
        ...
        defaults: RuffConfig = RuffAdapter.get_default_config()
    ...
    def _ruff_wizard_has_existing_config(self) -> bool:
        ...
        if not isinstance(tool, dict):
            return False
        excelsior = tool.get("excelsior", {})
        if not isinstance(excelsior, dict):
            return bool(tool.get("ruff"))
        return bool(tool.get("ruff") or excelsior.get("ruff"))
    ...
    def _ruff_wizard_prompt_use_defaults(self, defaults: RuffConfig) -> bool:
        ...
        lint_config = cast(dict[str, object], defaults['lint'])
        mccabe_config = cast(dict[str, object], lint_config['mccabe'])
        select_rules = cast(list[str], lint_config['select'])
        print(
            f"  • Max Complexity: {mccabe_config['max-complexity']}")
        print(
            f"  • Rule Categories: {len(select_rules)} enabled")
        print(f"    ({', '.join(select_rules[:8])}...)")
    ...
    def _ruff_wizard_customize(
        ...
        return (line_length, max_complexity, cast(list[str], lint_config["select"]))
    ...
    def _write_ruff_config(self, line_length: int, max_complexity: int, select_rules: list[str]) -> None:
        ...
        pyproject_path = Path("pyproject.toml")
        data: dict[str, Any] = {}  # TOML structure is inherently untyped
