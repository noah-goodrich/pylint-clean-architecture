...
if TYPE_CHECKING:
    from excelsior_architect.domain.entities import LinterResult, TransformationPlan
...
class ApplyFixesUseCase:
    ...
    def __init__(
        ...
        self._test_baseline: int | None = None
        ...
        for violation in violations:
            ...
            if rule:
                ...
                if plan_or_plans is None:
                    ...
                    plans.extend([p for p in plan_or_plans if p is not None])
                else:
        ...
        try:
            ...
            for violation in violations:
                ...
                if plan_or_plans is None:
                    ...
                    added: list[TransformationPlan] = [
                        p for p in plan_or_plans if p is not None and isinstance(p, TransformationPlan)]
                    plans.extend(added)
                    if added and rule_code not in rule_codes_out:
                        rule_codes_out.append(rule_code)
                elif isinstance(plan_or_plans, TransformationPlan):
                    plans.append(plan_or_plans)
    ...
    def _cleanup_backup_if_requested(self, backup_path_str: str | None) -> None:
