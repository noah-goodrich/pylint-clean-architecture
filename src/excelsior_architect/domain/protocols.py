...
class AstroidProtocol(Protocol):
    ...
    def get_return_type_qname_from_expr(self, expr: astroid.nodes.NodeNG) -> str | None:
        ...
...
class PythonProtocol(Protocol):
    ...
    def is_external_dependency(self, file_path: str | None) -> bool:
        ...
    ...
    def get_node_layer(self, node: astroid.nodes.NodeNG, config_loader: ConfigurationLoader) -> str | None:
        ...
...
class LinterAdapterProtocol(Protocol):
    ...
    def gather_results(self, target_path: str,
                       select_only: list[str] | None = None) -> list[LinterResult]: ...

    def apply_fixes(
        self, target_path: str, select_only: list[str] | None = None
    ) -> bool:
...
class AnalysisRendererProtocol(Protocol):
    """Protocol for rendering analysis results to a specific output target (CLI, Streamlit, etc.)."""
    ...
    def render_health_report(
        ...
        """Render full architectural health report."""
        ...
    ...
    def render_status(self, message: str, level: str) -> None:
        """Render a status message (e.g. info, warning, success)."""
        ...


class AnalysisPersistenceProtocol(Protocol):
    """Protocol for persisting analysis results to storage (files, Snowflake tables, etc.)."""

    def save_report(self, report: "ArchitecturalHealthReport") -> None:
        """Save full health report (e.g. last_audit.json)."""
        ...

    def save_ai_handover(self, report: "ArchitecturalHealthReport") -> str:
        """Generate and save AI handover bundle. Returns path or identifier."""
        ...


class SerializerProtocol(Protocol):
    """Protocol for serializing domain entities to a specific format (JSON, protobuf, etc.)."""

    def serialize(self, data: dict) -> str | bytes:
        """Serialize a dict (e.g. from entity.to_dict()) to string or bytes."""
        ...

    def deserialize(self, raw: str | bytes) -> dict:
        """Deserialize string or bytes to dict."""
        ...
...
class StubCreatorProtocol(Protocol):
    ...
    def extract_w9019_modules(self, linter_results: list[object]) -> set[str]:
...
class ScaffolderProtocol(Protocol):
    ...
    def init_project(
        self, template: str | None = None, check_layers: bool = False
    ) -> None:
