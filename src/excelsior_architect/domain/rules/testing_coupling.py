...
from excelsior_architect.domain.rules import StatefulRule, Violation
...
class TestingCouplingRule(StatefulRule):
    ...
    def record_functiondef(
        ...
        if not hasattr(node, "name") or not getattr(node, "name", "").startswith("test_"):
            return None
        return cast(astroid.nodes.FunctionDef, node)
    ...
    def record_mock_only(
        ...
        return bool(call_str and ("Mock(" in call_str or "MagicMock(" in call_str or "patch(" in call_str))
