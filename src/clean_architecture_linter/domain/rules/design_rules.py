...
class DesignRule:
    ...
    def check_functiondef(self, node: astroid.nodes.FunctionDef) -> list[Violation]:
        ...
        violations: list[Violation] = []
        if not getattr(node, "returns", None):
            violations.append(
                Violation.from_node(
    ...
    def _recursive_check_any(self, node: astroid.nodes.NodeNG, context: str) -> list[Violation]:
        violations: list[Violation] = []
        found_any = False
        if isinstance(node, astroid.nodes.Name) and getattr(node, "name", None) == "Any":
            found_any = True
        elif isinstance(node, astroid.nodes.Attribute) and getattr(node, "attrname", None) == "Any":
            found_any = True
    ...
    def _match_none_check(self, test: astroid.nodes.NodeNG | None) -> Optional[str]:
        if test is None:
            return None
        if isinstance(test, astroid.nodes.Compare) and len(getattr(test, "ops", [])) == 1:
            op, comparator = test.ops[0]
            if op in ("is", "is not") and isinstance(comparator, astroid.nodes.Const) and getattr(comparator, "value", None) is None:
                if isinstance(test.left, astroid.nodes.Name):
                    return str(test.left.name)
        if isinstance(test, astroid.nodes.UnaryOp) and getattr(test, "op", None) == "not":
            if isinstance(getattr(test, "operand", None), astroid.nodes.Name):
