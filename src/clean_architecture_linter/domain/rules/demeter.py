...
class LawOfDemeterRule:
    ...
    def _module_from_call(self, call_node: astroid.nodes.Call) -> Optional[str]:
        ...
        if isinstance(func, astroid.nodes.Name):
            stmts = getattr(func, "lookup", lambda _: ([], []))(func.name)[1]
            ...
            for target in assign.targets:
                ...
                if isinstance(assign.value.func, astroid.nodes.Attribute):
                    ...
                    if receiver_qname and self._ast_gateway.is_primitive(
                        ...
                        return True
                    if isinstance(
                        assign.value.func.expr, astroid.nodes.Name
                    ) and self._has_isinstance_primitive_guard(
                        func_node,
                        assign.value.func.expr.name,
                        assign.lineno,
                    ):
                        return True
        return False
    ...
    def _is_mock_involved(self, node: astroid.nodes.NodeNG) -> bool:
        ...
        if qname and any(
            m in qname for m in ("unittest.mock", "pytest", "MagicMock")
        ):
            return True
        return False
