...
class SnowflakeGovernanceChecker(BaseChecker):
    ...
    msgs = {
        "W9601": (
            "Select Star Violation: 'SELECT *' detected. Specify columns explicitly.",
            "select-star-violation",
            "Always specify explicit columns in SQL to prevent downstream breakages.",
        ),
        "W9602": (
            "Gold Layer View Warning: View detected in Gold layer. Prefer Tables or Dynamic Tables.",
            "gold-layer-view-violation",
            "Gold layer models should be materialized for performance and stability.",
        ),
        "W9603": (
            "Gold Layer Schema Evolution: Automatic schema evolution detected for Gold table.",
        ...
        for import_node in node.nodes_of_class((nodes.Import, nodes.ImportFrom)):
            if isinstance(import_node, nodes.Import):
                for name, _ in import_node.names:
                    if name.startswith("snowflake.connector") or name.startswith(
                        "snowflake.snowpark"
                    ):
                        self._is_snowflake_module = True
                        return
            elif isinstance(import_node, nodes.ImportFrom):
                if import_node.modname and (
                    import_node.modname.startswith("snowflake.connector")
                    or import_node.modname.startswith("snowflake.snowpark")
                    or import_node.modname.startswith(
                        "snowarch_core.infrastructure.snowflake"
                    )
                ):
                    self._is_snowflake_module = True
                    return
