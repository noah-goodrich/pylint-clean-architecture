...
class TestAstroidGateway(unittest.TestCase):
    ...
    def test_get_node_return_type_qname_const(self) -> None:
        ...
        self.assertEqual(
            self.gateway.get_node_return_type_qname(node), "builtins.int")
    ...
    def test_is_fluent_call(self) -> None:
        ...
        # Correct type implies has 'attrname' etc
...
"""
    ...
    def test_find_method_in_class_hierarchy_handles_exception(self) -> None:
        ...
        result = self.gateway._find_method_in_class_hierarchy(
            "MyClass", "method", context)
        assert result is None
    ...
    def test_find_method_in_class_hierarchy_handles_astroid_building_error(self) -> None:
        ...
        with patch(
            'astroid.MANAGER.ast_from_module_name',
            side_effect=astroid.AstroidBuildingError("Module not found")
        ):
            result = self.gateway._find_method_in_class_hierarchy(
                "nonexistent.Module", "method", context)
            assert result is None
    ...
    def test_resolve_method_in_node_handles_inference_error(self) -> None:
        ...
        mock_class.ancestors.side_effect = astroid.InferenceError(
            "Cannot infer")
    ...
    def test_find_attribute_type_in_class_functiondef_name_via_stub(self) -> None:
        ...
        self.gateway.typeshed.is_stdlib_module.return_value = False  # astroid is not stdlib
        mod_parse2 = astroid.parse(
            "class FunctionDef: pass\nx = 1\n", path=str(Path(__file__).resolve()))
        # ClassDef with no "name" AnnAssign in body
        empty_class = mod_parse2.body[0]
